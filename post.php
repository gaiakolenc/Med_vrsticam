...
$is_premium = (isset($_SESSION['user']['is_premium']) && $_SESSION['user']['is_premium'] == 1);
...
<head>
    ...
    <style>
        :root { --primary-coral: #FF6B6B; --dark-text: #2D3436; }
        body { margin: 0; font-family: 'Poppins', sans-serif; background-color: #fcfcfc; color: var(--dark-text); }
        
        .post-nav { position: fixed; top: 0; left: 0; right: 0; height: 80px; background: rgba(255, 255, 255, 0.9); backdrop-filter: blur(15px); z-index: 1000; display: flex; align-items: center; justify-content: center; padding: 0 40px; border-bottom: 1px solid rgba(0,0,0,0.05); }
        .nav-left { position: absolute; left: 40px; }
        .nav-right { position: absolute; right: 40px; }
        .logo-center { font-size: 18px; font-weight: 800; letter-spacing: 4px; text-transform: uppercase; text-decoration: none; color: var(--dark-text); }
        .logo-center span { color: var(--primary-coral); }

        .post-container { background: white; padding: 60px; border-radius: 40px; margin: 120px auto 50px auto; box-shadow: 0 20px 50px rgba(0,0,0,0.03); max-width: 850px; }
        
        .interaction-bar { display: flex; align-items: center; justify-content: space-between; margin: 40px 0; padding: 20px 30px; background: #fdfdfd; border: 1px solid #FFF9F2; border-radius: 30px; }
        .like-btn { background: <?= $ze_vseckal ? '#FF6B6B' : 'white' ?>; color: <?= $ze_vseckal ? 'white' : '#FF6B6B' ?>; border: 2px solid #FF6B6B; padding: 12px 25px; border-radius: 50px; cursor: pointer; font-weight: bold; display: flex; align-items: center; gap: 10px; transition: 0.3s; }
        
        .btn-aesthetic { padding: 12px 25px; border-radius: 15px; font-weight: 600; cursor: pointer; transition: 0.3s; display: inline-block; text-decoration: none; border: none; }
        .comment-textarea { width: 100%; border: 2px solid #FFF9F2; background: #FFF9F2; border-radius: 25px; padding: 20px; font-family: inherit; resize: none; box-sizing: border-box; }

        .premium-pdf-box { background: #FFF9F2; border: 1px solid var(--primary-coral); padding: 25px; border-radius: 25px; margin-top: 30px; text-align: center; }
...
<nav class="post-nav no-print">
    <div class="nav-left"><a href="index.php" class="btn-aesthetic" style="background: #f5f5f5; color: #333;">← Nazaj</a></div>
    <a href="index.php" class="logo-center">Med vrsticami<span>.</span></a>
    <div class="nav-right">
        <?php if ($is_premium): ?>
            <button onclick="window.print()" class="btn-aesthetic" style="background: var(--dark-text); color: white;">Shrani PDF 📄</button>
        <?php else: ?>
            <a href="https://buy.stripe.com/test_00wbJ10tq5Ce26w9F1b3q00" class="btn-aesthetic" style="background: #eee; color: #999;">PDF (Samo Premium 💎)</a>
        <?php endif; ?>
    </div>
</nav>
...
<div class="container">
    <article class="post-container">
        ...
        <?php if (!$is_premium): ?>
            <div class="premium-pdf-box no-print">
                <p style="margin: 0 0 10px 0; font-weight: 600;">Vam je zgodba všeč? Želite jo shraniti v PDF? 📄</p>
                <a href="https://buy.stripe.com/test_00wbJ10tq5Ce26w9F1b3q00" style="color: var(--primary-coral); font-weight: 800; text-decoration: none;">Postanite Premium član in odklenite prenose! 💎</a>
            </div>
        <?php endif; ?>
        ...
        <div class="interaction-bar no-print">
            ...
            <div class="qr-card" style="display: flex; align-items: center; gap: 10px;">
                <div style="text-align: right; font-size: 10px; color: #A0A0A0;">DELI ZGODBO</div>
                <img src="<?= $qr_api_url ?>" style="width: 50px; height: 50px; border-radius: 8px;">
            </div>
        </div>

        <section class="no-print" style="max-width: 750px; margin: 80px auto 0 auto;">
            <h3 style="font-family: 'Playfair Display'; font-size: 28px; margin-bottom: 40px;">
            ...
            <?php else: ?>
                <div style="text-align: center; padding: 40px; background: #FFF9F2; border-radius: 30px; margin-bottom: 40px;">
                    <p style="margin-bottom: 20px;">Želite sodelovati v debati?</p>
